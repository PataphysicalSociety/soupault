...
let section_matches site_dir actual_path conf_path =
...
let page_included options site_dir page_file =
  if (List.exists (regex_matches page_file) options.regexes_exclude) ||
     ...
     (List.exists (section_matches site_dir page_file) options.sections_exclude)
  then false
  ...
  | _, _, _ ->
    let should_run =
      ...
      (List.exists (section_matches site_dir page_file) options.sections)
    in
    let () =
