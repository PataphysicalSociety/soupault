...
let load_html state hooks page_file =
  ...
  let process_builtin_format settings file_name page_source =
    ...
    then !Markdown.render_markdown page_source
    else page_source
  in
...
let process_widgets state widget_list widget_hash page =
  ...
  let process_widget state widget_hash page widget_name =
    ...
    try widget.func state widget.config page
    with
...
let print_version () =
  ...
  print_endline "Copyright 2026 Daniil Baturin et al.";
...
let main cli_options =
  ...
  | BuildWebsite ->
    ...
    in
    (* Update the global Markdown renderer instance with actual settings. *)
    let () = Markdown.render_markdown := Markdown.make_markdown_renderer settings in
    ...
    (* Asset files are processed before pages
       so that widgets can access and manipulate them.
     *)
    ...
    (* Load and parse pages from disk for further processing. *)
    let pages = List.map (make_page settings) page_sources in
    (* Create target directories for pages from the site directory in advance,
     ...
     *)
    let () = Logs.info @@ fun m -> m "Creating directories for pages" in
    ...
    (* Extract metadata, if running in the generator mode. *)
    let index =
      if not settings.generator_mode
      then []
      else begin
    ...
    in
    (* Sort index entries according to the global settings sort option
       so that widgets that use index data don't have to sort it themselves. *)
    let index = Autoindex.sort_entries settings settings.index_sort_options index in
    (* In generator mode, run index processors, on all pages, content and index alike.
    ...
    (* Create target directories for autogenerated pages as well.
       Since we create page directories in advance to allow widgets to count on them,
       we need to do it for autogenerated pages separately —
       the directories created earlier for normal pages from the site directory
       may not accomodate all paths that autogenerated pages may use.
     *)
    let () =
      if not (CCList.is_empty new_pages)
      then Logs.info @@ fun m -> m "Creating directories for pages generated by index processors"
    in
    let () = List.iter create_page_dir new_pages in
