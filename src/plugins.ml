...
let list_plugins config =
  let ps = Config.get_table_opt Defaults.plugins_table config >>= (fun x -> Some (Toml_utils.list_table_keys x)) in
  match ps with
  | None -> []
  | Some ps' -> ps'
    ...
    begin
      ...
      | Some file ->
        try
          ...
          Hashtbl.add hash p (Plugin_api.run_plugin config file lua_source);
